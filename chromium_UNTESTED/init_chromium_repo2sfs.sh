...
MYFILENAME_DRIVER=chromedriver_68.0.3440.75-1~deb9u1_amd64
...
dpkg-deb -e $MYFILENAME.deb		#extract the DEBIAN folder to the current directory
cp DEBIAN/control "/tmp/repo2sfs/"$MYFILENAME"_control.txt"

echo "+++++++++++++ $MYFILENAME_DRIVER +++++++++++++++"
dpkg-deb -e $MYFILENAME_DRIVER.deb		#extract the DEBIAN folder to the current directory
cp DEBIAN/usr/bin/chromedriver "/tmp/repo2sfs/usr/bin/chromedriver"
cp DEBIAN/control "/tmp/repo2sfs/"$MYFILENAME_DRIVER"_control.txt"
